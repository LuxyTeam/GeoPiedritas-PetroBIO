...
import { Info, Box, Grid3X3, Gem, Layers } from 'lucide-react';

const DolomiteViewer = () => {
    const canvasRef = useRef(null);
    const containerRef = useRef(null);
    const [viewMode, setViewMode] = useState('supercell');
    ...
    const CONFIG = {
        baseScale: 30,
        colors: {
            Ca: { base: '#cbd5e1', shadow: '#475569', highlight: '#f1f5f9' }, // Calcio (Gris)
            Mg: { base: '#22c55e', shadow: '#14532d', highlight: '#86efac' }, // Magnesio (Verde vibrante)
        ...
        // El radio del Mg (0.72 A) es menor que el Ca (1.00 A)
        radii: { Ca: 13, Mg: 9, C: 7, O: 6 },
    ...
    useEffect(() => {
        ...
        // --- 1. Generación de Datos (Dolomita - Estructura ordenada) ---
        function initModel() {
            state.atoms = [];
            state.bonds = [];

            if (viewMode === 'unit') {
                // Unidad Representativa: Ca - CO3 - Mg - CO3
                // Mostramos el apilamiento vertical básico
                addAtom('Ca', 0, -3.5, 0);
                addCarbonate(0, -1.75, 0, false);
                addAtom('Mg', 0, 0, 0);
                addCarbonate(0, 1.75, 0, true);

            } else {
                // Red Cristalina (Capas alternas Ca/Mg)
                const latticeA = 4.6;
                const latticeC = 3.2; // Distancia entre capas verticales

                // Generamos 5 capas para ver el patrón: Ca - CO3 - Mg - CO3 - Ca
                // Índices: -2 (Ca), -1 (CO3), 0 (Mg), 1 (CO3), 2 (Ca)
                ...
                layers.forEach(l => {
                    ...
                    const isCationLayer = (l % 2 === 0);

                    // Si es capa de cationes, determinamos si es Ca o Mg
                    // En Dolomita, se alternan: Capa 0->Mg, Capa 2->Ca, Capa -2->Ca
                    // Lógica: Si (l/2) es par -> Mg (o Ca), si es impar -> el otro.
                    // Hagamos 0 = Mg, +/-2 = Ca
                    let cationType = 'Ca';
                    if (isCationLayer) {
                        // l=0 -> 0%2=0 (Mg)
                        // l=2 -> 1%2=1 (Ca)
                        // l=-2 -> -1%2=-1 (Ca)
                        cationType = (Math.abs(l / 2) % 2 === 0) ? 'Mg' : 'Ca';
                    }
                    ...
                    for (let q = -radius; q <= radius; q++) {
                        for (let r = -radius; r <= radius; r++) {
                            ...
                            if (isCationLayer) {
                                addAtom(cationType, x, layerY, z);
                            } else {
                                // Capa de Carbonato (rotada alternativamente)
        ...
        function draw() {
            ...
            state.bonds.forEach(bond => {
                ...
                if (a1 && a2) {
                    ...
                    renderList.push({ type: 'bond', z: zAvg, p1: a1.p, p2: a2.p, alpha: 1 });
                }
            });
        ...
        <div className="flex flex-col h-screen bg-slate-900 text-slate-100 font-sans overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row items-center justify-between px-4 py-3 md:px-6 md:py-4 bg-slate-800 border-b border-slate-700 z-10 shadow-lg gap-3 md:gap-0">
                ...
                <div className="flex items-center justify-center md:justify-start w-full md:w-auto text-center md:text-left">
                    <div>
                        <h1 className="text-lg md:text-xl font-bold tracking-tight">Dolomita CaMg(CO₃)₂</h1>
                        <p className="text-xs text-slate-400 font-medium">
                            {viewMode === 'supercell' ? 'Cristal Ordenado' : 'Unidad Fórmula'}
                        </p>
                    </div>
                </div>
                ...
                <div className="absolute bottom-4 left-4 right-4 md:right-auto md:bottom-6 md:left-6 bg-slate-800/90 backdrop-blur-md border border-slate-700 p-3 md:p-4 rounded-xl shadow-2xl md:max-w-xs pointer-events-none">
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center gap-3">
                            <div className="w-6 h-6 rounded-full bg-slate-300 shadow-inner border border-slate-400"></div>
                            <span className="text-sm font-medium text-slate-200">Calcio (Ca²⁺)</span>
                        </div>
                        <div className="flex items-center gap-3">
                            <div className="w-5 h-5 rounded-full bg-green-500 shadow-inner border border-green-400 ml-0.5"></div>
                            <span className="text-sm font-medium text-slate-200">Magnesio (Mg²⁺)</span>
                        </div>
                        <div className="flex items-center gap-3">
                            <div className="w-4 h-4 rounded-full bg-slate-700 shadow-inner border border-slate-500 ml-1"></div>
                            <span className="text-sm font-medium text-slate-200">Carbonato (CO₃²⁻)</span>
                        </div>
                    </div>
                    <div className="mt-4 pt-3 border-t border-slate-700 text-[10px] leading-tight text-slate-500">
                        <span className="text-green-300 flex items-center gap-1 mb-1"><Layers className="w-3 h-3" /> Alternancia de Capas:</span>
                        Observa cómo las capas de Calcio (Gris) y Magnesio (Verde) se alternan ordenadamente, separadas por grupos carbonato.
                    </div>
                </div>
            </div>
        </div>
